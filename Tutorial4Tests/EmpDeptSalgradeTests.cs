...
public class EmpDeptSalgradeTests
{
    ...
    public void ShouldReturnEmployeesFromChicago()
    {
        ...
        List<Emp> result; 
        result = emps.Where(e => depts.Any(d => d.Loc == "CHICAGO" && d.DeptNo == e.DeptNo)).ToList();
    ...
    public void ShouldJoinEmployeesWithDepartments()
    {
        ...
        var result = (from e in emps join d in depts
            on e.DeptNo equals d.DeptNo select new { e.EName, d.DName }).ToList();
        
        //possible
        var result1 = emps.Join(depts, e => e.DeptNo, d => d.DeptNo,
            (e, d) => new { e.EName, d.DName }).ToList();
        
        Assert.Contains(result1, r => r.DName == "SALES" && r.EName == "ALLEN");
